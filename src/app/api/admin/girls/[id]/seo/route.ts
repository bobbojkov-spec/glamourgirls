...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      try {
        console.log(`🔵 Attempting SEO UPDATE for girl ${actressId} with params:`, updateParams.map((p, i) => `${i + 1}: ${p?.substring?.(0, 30) || p || 'null'}`));
        const result = await client.query(updateQuery, updateParams);
        console.log(`✅ SEO data UPDATE executed for girl ${actressId}, rows affected: ${result.rowCount}`);
        
        if (result.rowCount === 0) {
          console.warn(`⚠️ UPDATE affected 0 rows for girl ${actressId} - girl may not exist`);
        }
        ...
        if (verifyResult.rows && verifyResult.rows.length > 0) {
          ...
          console.log(`✅ Verified SEO data saved for girl ${actressId}:`, {
          ...
          console.error(`❌ SEO data verification failed: No rows returned for girl ${actressId}`);
        }
      } catch (updateError: any) {
        // Log the actual error to see what's wrong
        console.error(`❌ SEO UPDATE error for girl ${actressId}:`, {
        ...
        if (updateError.code === '42703' || updateError.message?.includes('introtext') || updateError.message?.includes('column') && updateError.message?.includes('does not exist')) {
          ...
          console.log(`🔵 Attempting SEO UPDATE (without introText) for girl ${actressId} with params:`, updateParams.map((p, i) => `${i + 1}: ${p?.substring?.(0, 30) || p || 'null'}`));
          const result = await client.query(updateQuery, updateParams);
          console.log(`✅ SEO data UPDATE executed (without introText) for girl ${actressId}, rows affected: ${result.rowCount}`);
          ...
          if (verifyResult.rows && verifyResult.rows.length > 0) {
            const saved = verifyResult.rows[0];
            // PostgreSQL returns lowercase column names
            console.log(`✅ Verified SEO data saved (without introText) for girl ${actressId}:`, {
              seoTitle: saved.seotitle?.substring(0, 50) || 'null',
              metaDescription: saved.metadescription?.substring(0, 50) || 'null',
              h1Title: saved.h1title?.substring(0, 50) || 'null',
              metaKeywords: saved.metakeywords?.substring(0, 30) || 'null',
